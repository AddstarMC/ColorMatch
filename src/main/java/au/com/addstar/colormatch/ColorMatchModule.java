...
import au.com.addstar.colormatch.menu.MenuItemShowPatterns;
...
public class ColorMatchModule extends MinigameModule {
    ...
    static {
        setVersion(new ComparableVersion("1.13"));
    }

    private final IntegerFlag spectatorHeight;
    private final IntegerFlag floorDepth;
    private final IntegerFlag fallDepth;
    private final IntegerFlag superSigns;
    private final StringFlag idleMessage;
    private final IntegerFlag roundsPerGame;
    private final IntegerFlag roundWaitTime;
    private final IntegerFlag postGameTime;
    private final FloatFlag initialRoundTime;
    private final IntegerFlag roundSpan;
    private final FloatFlag minimumRoundTime;
    private final IntegerFlag roundDelay;
    private final BooleanFlag extendedColors;

    private final MaterialGroupFlag materialGroup;
    ...
    public ColorMatchModule(Minigame mgm) {
        ...
        addFlag(materialGroup = new MaterialGroupFlag(MaterialGroupRegistry.getDefaultGroup(), "material"));
    ...
    public void save(FileConfiguration config) {
        if (game != null) {
            ...
            for (WeightedPatternMap.WeightedPattern pattern : game.getPatternMap().getPatterns()) {
        ...
        if (config.isConfigurationSection("board")) {
            ...
            if (patternStrings != null) {
                for (String string : patternStrings) {
                    ...
                    if (pattern != null) {
                        game.getPatternMap().add(new WeightedPatternMap.WeightedPattern(weight, name, pattern));
                    }
                }
            }
    ...
    public boolean displayMechanicSettings(Menu previous) {
        ...
        menu.addItem(materialGroup.getMenuItem("Board Material", Material.BROWN_CONCRETE_POWDER, Arrays
                .asList("The base material group the board", "is made of. The actual materials of that group will" + " be randomized")));
        ...
        if (game != null)
            menu.addItem(new MenuItemShowPatterns("Edit Patterns", Material.GREEN_GLAZED_TERRACOTTA, game));
    ...
    public MaterialGroup getBoardMaterial() {
        if (materialGroup.getFlag() == null) {
            return materialGroup.getDefaultFlag();
        }
        return materialGroup.getFlag();
    }
