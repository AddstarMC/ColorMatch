...
public class ColorMatch extends JavaPlugin implements Listener {

    static MaterialGroupRegistry groups;
    public static Economy econ = null;
    public static SignMakerPlugin signmaker = null;
    public static ColorMatch plugin;
    private PatternSelectionTool tool;
    @Override
    public void onEnable() {
        plugin = this;
        if(!ColorMatchModule.checkVersion()){
            Minigames.getPlugin().getLogger().warning(ColorMatchModule.class.getCanonicalName() +" cannot be added as it requires Minigames version: " + ColorMatchModule.getMinRequired().getCanonical());
            return;
        }
        getServer().getPluginManager().registerEvents(this, this);
        GameMechanics.addGameMechanic(new ColorMatchMechanic());
        tool = new PatternSelectionTool();
        ToolModes.addToolMode(tool);

        PatternRegistry.loadSaved();
        MaterialGroupRegistry.loadGroups();
    ...
    public void onDisable() {
        ...
        ToolModes.removeToolMode(tool.getName());
        tool = null;
    }
